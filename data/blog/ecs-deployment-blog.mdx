---
title: 'Complete Guide: Deploying Spring Boot API on AWS ECS'
date: '2024-10-29'
...
graph TB
    ...
    subgraph VPC[VPC 10.0.0.0/16]
        ...
        subgraph AZ2[Availability Zone 2]
            subgraph Public2[Public Subnet 10.0.3.0/24]
                ALB2[ALB Replica]
            end
...
graph TB
    subgraph "Availability Zones"
        ...
        subgraph "AZ-2"
            PUB2[Public 10.0.3.0/24]
            PRIV2[Private 10.0.4.0/24]
        end
    end
    RT1[Public Route Table] --> PUB1
    RT1 --> PUB2
    RT2[Private Route Table 1] --> PRIV1
    RT3[Private Route Table 2] --> PRIV2
    NG1[NAT Gateway 1] --> PRIV1
    NG2[NAT Gateway 2] --> PRIV2
`}
/>
...
<MermaidChart
  chart={`
graph LR
    Internet((Internet)) --> |Port 80/443| ALB_SG[ALB Security Group]
    ALB_SG --> |Port 8080| ECS_SG[ECS Security Group]
    ECS_SG --> |Egress All| Internet
`}
/>
...
graph TB
    ...
    subgraph Load Balancer Components
        ALB --> List80[HTTP Listener :80]
        ALB --> List443[HTTPS Listener :443]
        
        List80 -->|Redirect| List443
        List443 --> TG[Target Group]
        ...
        subgraph Target Group
            ...
            subgraph Health Checks
                T1 -->|/actuator/health| HC1[Health Check]
                T2 -->|/actuator/health| HC2[Health Check]
                T3 -->|/actuator/health| HC3[Health Check]
            end
        end
    end
...
sequenceDiagram
    ...
    CI->>CI: Build Image
    CI->>ECR: Push Image
    Note over ECR: Image Scanning
    ECS->>ECR: Pull Image
    Note over ECS: Deploy Container
    
    loop Health Check
        ECS->>ECS: Monitor Container Health
    end
`}
/>
